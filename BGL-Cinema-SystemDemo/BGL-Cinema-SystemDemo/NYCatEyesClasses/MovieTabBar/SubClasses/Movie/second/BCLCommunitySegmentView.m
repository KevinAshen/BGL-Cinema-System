...
- (void)creatSegmentView {
    ...
    //添加一个select
    _selectImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, self.frame.size.height - 2, _itemWidth, 2)];
...
-(void)buttonAction:(UIButton *)button{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self->_selectImage.frame = CGRectMake(index * self->_itemWidth, _selectImage.frame.origin.y, self->_selectImage.frame.size.width, _selectImage.frame.size.height);
    }];
